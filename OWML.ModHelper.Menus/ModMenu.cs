...
namespace OWML.ModHelper.Menus
{
    public class ModMenu : IModMenu
    {
        ...
        private T GetTitleButton<T>(string title, List<T> buttons) where T : IModButton
        {
            ...
            if (button == null)
            {
                OwmlConsole.WriteLine(MessageType.Warning, "Warning : No button found with title or name: " + title);
            }
            return button;
        }
        ...
        public object GetInputValue(string key)
        {
            ...
            OwmlConsole.WriteLine(MessageType.Error, "Error - No input found with name " + key);
            return null;
        }
