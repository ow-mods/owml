...
namespace OWML.EnableDebugMode
{
    public class EnableDebugMode : ModBehaviour
    {
        ...
        private void Start()
        {
            ModHelper.Console.WriteLine(MessageType.Log, $"In {nameof(EnableDebugMode)}!");
        ...
        private void CycleGUIMode()
        {
            ...
            ModHelper.Console.WriteLine(MessageType.Log, "Render value: " + _renderValue);
            typeof(GUIMode).GetAnyField("_renderMode").SetValue(null, _renderValue);
        }

        private void WarpTo(SpawnLocation location)
        {
            ModHelper.Console.WriteLine(MessageType.Log, $"Warping to {location}!");
