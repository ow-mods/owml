...
namespace OWML.ModLoader
{
    internal class Owo
    {
        ...
        public void LoadMods()
        {
            ...
            {
                _console.WriteLine(MessageType.Log, "Verbose mode is enabled");
            ...
            foreach (var modData in sortedMods)
            {
                var missingDependencies = modData.Config.Enabled ?
                    ...
                    new List<string>();
                missingDependencies.ForEach(dependency => _console.WriteLine(MessageType.Error,
        ...
        private void OnLogMessageReceived(string message, string stackTrace, LogType type)
        {
            ...
            {
                _console.WriteLine(MessageType.Error, $"Unity log message: {message}. Stack trace: {stackTrace?.Trim()}");
            }
        }
        ...
        private Type LoadMod(IModData modData)
        {
            ...
            {
                _console.WriteLine(MessageType.Error, $"Error while trying to get {typeof(ModBehaviour)}: {ex.Message}");
                return null;
            }
        }
        ...
        private IModBehaviour InitializeMod(Type modType, IModHelper helper)
        {
            ...
            {
                _console.WriteLine(MessageType.Error, $"Error while adding/initializing {helper.Manifest.UniqueName}: {ex}");
                return null;
            }
        }
